  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden transition-shadow duration-300">
      ...
      <div className="p-6">
        ...
        <p className="text-light-text text-sm mb-4 line-clamp-3">
          {contribution.description}
        </p>

        {contribution.target_amount &&
          parseFloat(contribution.target_amount) > 0 && (
            <div className="mb-4">
        ...
        (contribution.current_amount &&
          parseFloat(contribution.current_amount) > 0) ? (
            ...
            {contribution.current_amount &&
              parseFloat(contribution.current_amount) > 0 && (
                <div className="flex items-center text-light-text">
                  ...
                  <span>
                    Raised: {formatCurrency(contribution.current_amount)}
                  </span>
                </div>
              )}
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link
            to={`/endowment/${contribution.id}?source=${
              contribution._source || "test_app"
            }`}
  ...
  useEffect(() => {
    const fetchEndowment = async () => {
      try {
        ...
        const [scholarshipsAppResponse, testAppResponse] = await Promise.all([
          apiClient.get("/scholarships/list/").catch((err) => {
            console.error("Error fetching from scholarships app:", err);
            return { data: [] };
          }),
          apiClient.get("/endowment/").catch((err) => {
            console.error("Error fetching from test_app:", err);
            return { data: { results: [] } };
          }),
        ]);

        // Mark source for each scholarship
        const scholarshipsAppData = (scholarshipsAppResponse.data || []).map(
          (item) => ({
            ...item,
            _source: "scholarships_app",
          })
        );
        const testAppData = (
          testAppResponse.data.results ||
          testAppResponse.data ||
          []
        ).map((item) => ({
          ...item,
          _source: "test_app",
        ...
        // Combine both sources - scholarships app first, then test_app
        const combinedEndowment = [...scholarshipsAppData, ...testAppData];

        setEndowment(combinedEndowment);

        // Calculate totals from combined data
        const totalRaised = combinedEndowment.reduce((sum, item) => {
          return sum + (parseFloat(item.current_amount) || 0);
        }, 0);

        const totalContributors = combinedEndowment.reduce((sum, item) => {
          return sum + (item.contribution_count || 0);
        }, 0);

        setTotals({
          total_raised: totalRaised,
          total_contributors: totalContributors,
          total_scholarships: combinedEndowment.length,
        });
      } catch (error) {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-light-text max-w-2xl mx-auto">
            Support the next generation of innovators and leaders. Your
            endowment help students achieve their academic dreams at IIT
            Kharagpur.
          </p>
        </div>
