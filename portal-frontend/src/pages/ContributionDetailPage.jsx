...
function ContributionDetailPage() {
  ...
  useEffect(() => {
    const fetchContributionDetails = async () => {
      try {
        ...
        // Get source from URL query parameter
        const urlParams = new URLSearchParams(window.location.search);
        const source = urlParams.get("source") || "test_app";

        let scholarshipResponse, contributorsResponse;

        // Fetch from the appropriate endpoint based on source
        if (source === "scholarships_app") {
          // Fetch from scholarships app
          scholarshipResponse = await apiClient.get(
            `/scholarships/list/${contributionId}/`
          );
          // Scholarships app doesn't have endowment/contributors yet, so use empty array
          contributorsResponse = { data: [] };
        } else {
          // Fetch from test_app (default)
          scholarshipResponse = await apiClient.get(
            `/endowment/${contributionId}/`
          );
          contributorsResponse = await apiClient.get(
            `/endowment/${contributionId}/endowment/list/`
          );
        }
  ...
  const handleDonate = async () => {
    // Check if source is from scholarships_app (which doesn't support endowments yet)
    const urlParams = new URLSearchParams(window.location.search);
    const source = urlParams.get("source") || "test_app";

    if (source === "scholarships_app") {
      alert(
        "Contributions are not yet available for this scholarship. This feature is coming soon!"
      );
      return;
    }
    ...
    try {
      ...
      await apiClient.post(
        `/endowment/${contributionId}/endowment/`,
        contributionData
      );

      alert(
        `Thank you for your donation of ${formatCurrency(donationAmount)} to ${
          contribution.title
        }!`
      );
  ...
  const generateShareText = () => {
    const urlParams = new URLSearchParams(window.location.search);
    const source = urlParams.get("source") || "test_app";
    const sourceParam = source !== "test_app" ? `?source=${source}` : "";
    const contributionUrl = `${window.location.origin}/endowment/${contribution.id}${sourceParam}`;
    return `🎓 ${contribution.title}\n\n${
      contribution.description
    }\n\n💰 Target: ${formatCurrency(
      contribution.target_amount
    )}\n📈 Progress: ${contribution.progress_percentage.toFixed(
      1
  ...
  const shareOnLinkedIn = () => {
    ...
    const linkedinUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(
      window.location.origin + "/endowment/" + contribution.id
        ...
        <div className="container mx-auto px-4">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">
              The contribution you're looking for doesn't exist or has been
              removed.
            </p>
            <Link
              to="/endowment"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6 lg:space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-light-text">
                    <div className="flex items-center">
                      ...
                      <span className="text-sm sm:text-base">
                        Created {formatDate(contribution.created_at)}
                      </span>
                    </div>
                ...
                <span
                  className={`px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-semibold self-start sm:self-auto ${
              ...
              {contribution.long_description &&
                contribution.long_description !== contribution.description && (
                  <div className="prose max-w-none">
                    <h3 className="text-lg sm:text-xl font-bold text-dark-text mb-4">
                ...
                <div className="flex items-center w-full max-w-xl">
                  ...
                  <div className="flex flex-col items-center flex-1">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${
                        ["REGISTRATION", "SELECTION", "FUND_RAISE"].includes(
                          contribution.status
                        )
                  ...
                  <div
                    className={`flex-1 h-1 ${
                      ["SELECTION", "FUND_RAISE"].includes(contribution.status)
                        ? "bg-blue-500"
                        : "bg-gray-300"
                    } mx-2`}
                  ></div>
                  ...
                  <div
                    className={`flex-1 h-1 ${
                      contribution.status === "FUND_RAISE"
                        ? "bg-blue-500"
                        : "bg-gray-300"
                    } mx-2`}
                  ></div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 sticky top-4 sm:top-8">
              {/* Progress */}
              <div className="mb-6">
                <h3 className="text-base sm:text-lg font-bold text-dark-text mb-4">
                  Funding Progress
                </h3>
                <div className="mb-4">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2 sm:h-3">
                    <div
                      ...
                      style={{
                        width: `${Math.min(
                          100,
                          contribution.progress_percentage
                        )}%`,
                      }}
                    ></div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
                  <div>
                    ...
                    <p className="font-bold text-dark-text text-sm sm:text-base">
              ...
              <div className="mb-6 space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {contribution.contribution_count > 0 ? (
                    ...
                    <span className="font-bold text-dark-text text-sm sm:text-base">
                      0
                    </span>
                  )}
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-dark-text text-sm sm:text-base">
                    {formatCurrency(contribution.remaining_amount)}
                  </span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm sm:text-base text-light-text mt-1 truncate">
                  {contribution.contribution_count} contributors •{" "}
                  {formatCurrency(contribution.current_amount)} raised
                </p>
              </div>
              <button
