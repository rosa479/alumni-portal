...
import { Link, useNavigate } from "react-router-dom"; // ✅ real navigation

// --- Mock Authentication Context ---
const useAuth = () => ({
  login: (access, refresh) => {
    console.log("Logged in with tokens:", { access, refresh });
    alert("✅ Login successful! Redirecting to dashboard...");
  },
});

// --- Mock API Client (simulating backend responses) ---
const apiClient = {
  post: (url, data) =>
      ...
      setTimeout(() => {
        if (url.includes("/auth/check-email/")) {
          if (data.email === "user@example.com") {
            resolve({ data: { message: "Email exists" } });
          } else {
          ...
          if (data.email === "user@example.com" && data.password === "password123") {
            resolve({ data: { access: "fake_access", refresh: "fake_refresh" } });
          } else {
